...
new WebSocket.Server({
            ...
            case 0:
                if (!user.startedBots) {
                    ...
                    startBotsInterval = setInterval(() => {
                        if (dataBot.lastPlayersAmount < 195 && connectedBots < bots.amount 96 !stoppingBots) userBots.push(new Bot())
                    }, 150)
    ...
    ws.on('close', () => {
        if (user.startedBots 96 !stoppingBots) {
            stoppingBots = true
            let seconds = 0
            setInterval(() => {
                if (seconds === 5) process.exit()
                else {
                    logger.warn(`[SERVER] Stopping bots in ${5 - seconds} seconds`)
                    seconds++
                }
            }, 1000)
        }
